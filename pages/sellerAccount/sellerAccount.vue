<template>
	<view class="container">
		...
		<image class="add_image" src="../../static/add_seller_btn.png" @click="addSellerAccount"></image>
	</view>
</template>
...
export default {
	...
	methods: {
		...
		async getData() {
			const { page, page_size, sub_id, role, list ,name} = this;
		...
		toDetail(user_id) {
			uni.navigateTo({
				url: '../acountDetail/acountDetail?id=' + user_id
			});
		},
		addSellerAccount() {
			uni.navigateTo({
				url: '../addSellerManager/addSellerManager'
			});
		},
		scrolltolower() {
			console.log(1);

		...
		<view class="info_box">
			...
			<view class="agency_detail">
				<view class="agency_left">
					<text class="info_title">{{$t('user-name')}}</text>
			...
			<view class="agency_use">
				<view class="logo_box" @click="editSeller">
					...
					<text class="text">{{$t('modify-staff')}}</text>
				</view>
				<view class="logo_box" @click="bindDevice">
					<image src="../../static/seller_bind.png"></image>
					<text class="text">{{$t('bind-the-device')}}</text>
				</view>
<!-- 				<view class="logo_box" @click="toSeller">
					<image src="../../static/selle_manger.png"></image>
					<text class="text">{{$t('store')}}</text>
				</view> -->
			</view>
		</view>
...
<script>
	...
	import {userDetail, userDelete,statDevice} from '../../utils/role/role'
	export default {
		data() {
			return {
				user_id: '',
		...
		methods: {
			...
			editSeller() {
				uni.navigateTo({
					url:`/pages/addEmployee/addEmployee?id=${this.user_id}&info=${JSON.stringify(this.info)}`
				})
			},
			async getInfo() {
...
.container {
	...
	.info_box {
		...
		.agency_use {
			display: flex;
			width: 100vw;
			padding: 24rpx 46rpx 36rpx 24rpx;
			box-sizing: border-box;
			.logo_box {
				display: flex;
				flex-direction: column;

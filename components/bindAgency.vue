<template>
	<view class="bindAgency">
		...
		<view class="input_box">
			...
			<input class="input" type="text" placeholder-class="phcolor" :placeholder="$t('please-enter-a-keyword')" @input="handleKeyword" />
		</view>
		 <view class="list">
			<radio-group @change="handleAgency">
				<view class="item" v-for="item in userList" :key="item.user_id">
					<text class="name">{{item.name}}</text>
					<view class="radio">
						<radio color="#3D82FF" :value="String(item.user_id)" ></radio>
					</view>
				</view>
			</radio-group>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			sub(){
			  this.$emit('selected',this.agency_id)
			  this.$emit('cancle')
			},
			cancle(){
			  this.$emit('cancle')
			...
			selectRole(e){
				...
				this.$emit('roleList',this.userList)
			},
			handleAgency(e) {
				this.agency_id = e.detail.value
